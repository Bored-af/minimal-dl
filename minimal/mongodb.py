...
client = MongoClient(
    "mongodb+srv://brayo:EasyPass@brayos.gmypf.mongodb.net/spotify?retryWrites=true&w=majority"
)
db = client.spotify
...
def unset_link_entry(id: str) -> bool:
    result = spotify_db.find_one_and_update(
        {"spotify_id": f"{id}"}, {"$set": {"yt_url": ""}}
    )
    if result != None:
        return True
    return False
...
def insert_genius_link(id: str, genius_url: str) -> bool:
    if spotify_db.find_one({"spotify_id": f"{id}"}) != None:
        result = spotify_db.find_one_and_update(
            {"spotify_id": f"{id}"}, {"$set": {"genius_url": f"{genius_url}"}}
        )
        # print(genius_url)
        if result != None:
            return True
